...
FilesystemOTA::FilesystemOTA(AsyncWebServer* srv, DebugLogger* log) 
    : server(srv), logger(log) {}

void FilesystemOTA::begin() {
    ...
    logger->info("FilesystemOTA endpoints initialized");
}

void FilesystemOTA::handleFilesystemUpload(AsyncWebServerRequest *request, 
                            String filename, 
                            size_t index, 
                            uint8_t *data, 
                            size_t len, 
    ...
    if (!index) {
        ...
        // Размер раздела LittleFS из partitions.csv (0xF0000 = 960KB)
        size_t fsSize = 0xF0000;
...
void FilesystemOTA::handleFileUpload(AsyncWebServerRequest *request,
                     String filename,
                     size_t index,
                     uint8_t *data,
                     size_t len,
...
void FilesystemOTA::handleFileList(AsyncWebServerRequest *request) {
    String path = "/";
    if (request->hasParam("path")) {
